function FILENAME = CanteraCH4(Y_f_0,Y_o_0,Y_p_0,tin,p)
% CanteraCH4() Generates a flame structure file solving a counter-flow
% diffusion flame with the Cantera library.
% INPUT:
% Y_f_0 = CH4 mass fraction in fuel stream
...
fuelcomp   =  append('CH4:',num2str(X_f_0),', N2:',num2str(1-X_f_0-0.01),', H2:',num2str(0.01));                    % Fuel composition
...
l = speciesIndex(fuel, 'CH4');          % Get index of H2 in gas object
...
for count = 1:1:length(FLAMESTRUCTURE.Z)
    mixcomp = append('CH4:',num2str(FLAMESTRUCTURE.YF(count)),', O2:',num2str(FLAMESTRUCTURE.YO(count)),', N2:',num2str(FLAMESTRUCTURE.YN(count)),', H2O:',num2str(FLAMESTRUCTURE.YP(count)));
...
%FLAMESTRUCTURE.D = FLAMESTRUCTURE.D./2;
%tfl = max(FLAMESTRUCTURE.T,[],'all');
%temp = 0.5*(FLAMESTRUCTURE.T-tin)/(tfl-tin)+1;
FLAMESTRUCTURE.D = interp1([0 Z_st 1],[1 1.5 1],FLAMESTRUCTURE.Z);
